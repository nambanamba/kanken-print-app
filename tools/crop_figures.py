...
# 本のスキャンは 300dpi（p19 の画像 2416×3508 を実測）。字1つは本来 120px ほどしかない。
# 600dpi で切り出して（字1つ 240px ほど）、アプリでは画面いっぱい＋タップで2倍に広げて見せる。
# ⚠️ 書き起こし担当の実測: 太い画が13字すべてで見分けられたのは「字1つ 560px 以上」で見たとき
#    （械・博・協・倉 は小さいと分からない）。それより上げても元のスキャン以上の細かさは出ないので、
#    大きさは「表示で広げる」で稼ぐ（アプリの ap-fig とタップで拡大）
DPI = 600
