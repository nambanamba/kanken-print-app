...
// 書き取りの自己申告の画面を撮る
await p.evaluate(() => { while (SESSION.items[SESSION.pos] && SESSION.items[SESSION.pos].field !== "kaki") SESSION.pos++; renderKyou(); });
await p.screenshot({ path: "tools/shot_kyou_kaki.png", fullPage: true });
...
let g = 0;
while (g++ < 400) {
  const st = await p.evaluate(() => ({ done: SESSION.done, f: (SESSION.items[SESSION.pos] || {}).field }));
  if (st.done) break;
  if (st.f === "kaki") await p.click('button:has-text("あやしい")');
  else { await p.evaluate(() => document.querySelectorAll("#ky-choices .ky-btn")[0].click()); await p.waitForTimeout(650); }
}
...
console.log("撮影しました: shot_kyou_start / _kaki / _select / _end / shot_kiroku / shot_ouen / shot_print");
