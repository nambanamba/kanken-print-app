...
//     常用漢字表の例語だけでは選択肢が作れないため。**問題集待ちではない**（2026-09-09に162ページ読了ずみ。
//     形式は把握できている）。順に足していく。まず「じゅく語作り」…読みが要らないので先に作れる。
//
...
function genBushu(k, rnd, avoid) {
  ...
  // ★部首名は「答え」ではなく、画面に添えるラベル。
  //   7級の審査基準は《部首》「部首を理解している。」だけで、**名前は問われない**
  //   （公式サイトの逐語＋実物の過去問(八)「同じ部首のなかまの漢字」の2つで確認済み。
  //     `公式資料\漢検公式サイト_採点基準と7級審査基準_2026-09-09.md`）。
  //   だから **642字ぶん集める必要はない。** 分かっている字にだけ出す。
  //   ⚠️ 名前が無い字は **hint ごと出さない。**「部首（）」のような空の括弧を作らないこと。
  //      642字中301字は名前が空のままなので、必ず誰かが踏む。
  var q = {
